...
def _load_and_train(model_inst, param_grid, data_dir, output_dir, model_name):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        for i, data_file in enumerate(data_files):
            ...
            future = executor.submit(_train_and_save, model_inst, param_grid, file_path, ticker, output_dir, model_name)
            futures.append(future)
        concurrent.futures.wait(futures)


def _train_and_save(model_inst, param_grid, file_path, ticker, output_dir, model_name):
    ...
    estimator, _ = util.evaluate_tr_model(model_inst, param_grid, X_train, y_train, X_test, y_test)
...
def main(_):
    ...
    model_inst, param_grid = util.get_model(best_model_name)
    _load_and_train(model_inst, param_grid, FLAGS.data_dir, output_dir)
